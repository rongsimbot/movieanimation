...
import { GripVertical, X, Plus, MapPin, Cloud, Users, Film, List } from 'lucide-react';
...
interface Scene {
  ...
  action: string;
  order: number;
  scriptItems: number[]; // Array of script item IDs
}

const parseScriptItems = (scriptText: string): ScriptItem[] => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^(INT\.|EXT\.)/.test(line)) {
      currentSceneId = Math.random().toString(36).substring(2);
      items.push({
        id: itemId++,
        type: 'scene',
        text: line,
    ...
    // Character name
    else if (line === line.toUpperCase() && /^[A-Z\s]+$/.test(line) && line.length > 1) {
      items.push({
        id: itemId++,
        type: 'dialogue',
        character: line,
    ...
    // Action or dialogue text
      ...
      if (lastItem && lastItem.type === 'dialogue' && !lastItem.text) {
        // Append to last dialogue
        lastItem.text = line;
      } else {
        // Action line
        items.push({
...
const parseScenes = (scriptText: string, scriptItems: ScriptItem[]): Scene[] => {
  const lines = scriptText.split('\n');
  const scenes: Scene[] = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^(INT\.|EXT\.)/.test(line)) {
      // Save previous scene
      if (currentScene && currentScene.title && currentSceneId) {
        const sceneItems = scriptItems
          .filter(item => item.sceneId === currentSceneId)
          .map(item => item.id);
        ...
        scenes.push({
          ...
          order: sceneOrder++,
          id: currentSceneId,
          scriptItems: sceneItems
        } as Scene);
      }

      // New scene
      currentSceneId = scriptItems.find(item => item.text === line && item.type === 'scene')?.sceneId || Math.random().toString(36).substring(2);
...
const formatRanges = (numbers: number[]): string => {
  if (numbers.length === 0) return 'No items';
...
const getWeatherIcon = (weather: string) => {
  const icons: Record<string, string> = {
    CLEAR: '☀️', RAIN: '🌧️', SNOW: '❄️', WIND: '💨',
    FOG: '🌫️', STORM: '⛈️', CLOUDY: '☁️'
  };
...
export const SceneManager: React.FC<{ script: string; onUpdate: (scenes: Scene[]) => void }> = ({ script, onUpdate }) => {
  ...
  useEffect(() => {
    const items = parseScriptItems(script);
    const parsedScenes = parseScenes(script, items);
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/50">{scenes.length} scenes • {scriptItems.length} script items</p>
          </div>
        </div>
        <button 
          onClick={addScene}
