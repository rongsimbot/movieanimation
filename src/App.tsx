...
const MovieSection = ({ title, movies }: { title: string, movies: SampleMovie[] }) => {  const navigate = useNavigate();    return (    <section className="py-12 px-6 md:px-20">      <div className="flex items-center justify-between mb-8">        <h2 className="text-2xl font-bold">{title}</h2>        <button className="text-brand-primary text-sm font-bold hover:underline">View all</button>      </div>      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">        {movies.map((movie) => (          <motion.div             key={movie.id}            whileHover={{ scale: 1.05 }}            onClick={() => navigate(`/player/animation/${movie.id}`)}            className="group relative aspect-[2/3] rounded-xl overflow-hidden cursor-pointer"          >            <img               src={movie.thumbnail}               alt={movie.title}               className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"              referrerPolicy="no-referrer"            />            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4">              <p className="text-xs font-bold text-brand-primary uppercase tracking-widest mb-1">{movie.category}</p>              <h3 className="text-sm font-bold leading-tight">{movie.title}</h3>            </div>          </motion.div>        ))}      </div>    </section>  );};
